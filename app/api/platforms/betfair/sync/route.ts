...
/**
 * GET /api/platforms/betfair/sync
 * Sync data from Betfair
 */
  ...
  try {
    ...
    if (accountError || !account) {
      return NextResponse.json(
        { error: "Betfair account not connected" },
        { status: 404 },
      );
    }
    ...
    if (account.expires_at && new Date(account.expires_at) < new Date()) {
      if (!refreshToken) {
        ...
        return NextResponse.json(
          { error: "Token expired. Please reconnect your Betfair account." },
          { status: 401 },
        );
      }
    ...
    const client = new BetfairClient({ accessToken });
    ...
    try {
      ...
      console.error("Betfair sync error:", syncError);
      const errorMessage =
    ...
    console.error("Betfair sync error:", error);
    return NextResponse.json(
